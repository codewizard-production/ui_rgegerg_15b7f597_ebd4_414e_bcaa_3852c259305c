...
const Component = (props) => {
    ...
    const OnCloseClicked = async (e) => {
        if (e) {
            const rslt = await Api.SetBikeSingle({ BikeID: deletedId, Deleted: true });
            if (rslt.status) {
                setInitialize(true);
    ...
    const OnActionClicked = (id, type) => {
        let _route;
        if (type === 'edit') _route = `/Bikes/edit/${id}`;
        if (type === 'view') _route = `/Bikes/view/${id}`;
                    ...
                    <Stack direction="row">
                        <Grid container sx={{ justifyContent: 'flex-end' }}>
                            ...
                            <IconButton
                                ...
                                onClick={() => NavigateTo("/Bikes/create")}
                            >
                                <AddBoxIcon />
                            </IconButton>
                        </Grid>
